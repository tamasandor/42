...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/02 15:44:05 by atamas            #+#    #+#             */
/*   Updated: 2025/12/01 20:59:09 by atamas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T> void PmergeMe::_merge_insertion_sort(T& container, int pair_level)
{
	...
	/* Initialize the main chain with the {b1, a1}.
		b1 is going to be the smallest element in the main chain. */
	...
	for (int k = 2;; k++)
	{
		...
		while (nbr_of_times)
		{
			typename std::vector<Iterator>::iterator idx =
				std::upper_bound(main.begin(), bound_it, *pend_it, PmergeMe::_comp<Iterator>);
	...
	for (ssize_t i = pend.size() - 1; i >= 0; i--)
	{
		...
		typename std::vector<Iterator>::iterator idx =
			std::upper_bound(main.begin(), curr_bound, *curr_pend, PmergeMe::_comp<Iterator>);
